# Datasite MCP Connector — DataRoom Skills

Eight coordinated Claude skills for sell-side M&A deal teams using Datasite as their Virtual Data Room platform.
...
| 2 | Smart File Renaming | Standardise document names across the data room | ✅ | ✅ |
...
- Datasite MCP Connector installed and authenticated (Blueflame AI)
...
Crawls the data room, identifies files with uninformative or inconsistent names, and proposes standardised names following deal conventions (e.g. `[Company] - Audited Accounts - FY2025.pdf`). Falls back to filename-pattern mode with placeholders if Blueflame is not enabled.
